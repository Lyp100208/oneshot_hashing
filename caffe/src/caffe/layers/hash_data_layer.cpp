...
void HashDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  const int new_height = this->layer_param_.hash_data_param().new_height();
  const int new_width  = this->layer_param_.hash_data_param().new_width();
  const bool is_color  = this->layer_param_.hash_data_param().is_color();
  string root_folder = this->layer_param_.hash_data_param().root_folder();

  const int label_size = this->layer_param_.hash_data_param().label_size();
  ...
  if (this->layer_param_.hash_data_param().rand_skip()) {
    unsigned int skip = caffe_rng_rand() %
        this->layer_param_.hash_data_param().rand_skip();
...
void HashDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
  ...
  HashDataParameter hash_data_param = this->layer_param_.hash_data_param();
  const int batch_size = hash_data_param.batch_size();
  const int new_height = hash_data_param.new_height();
  const int new_width = hash_data_param.new_width();
  const bool is_color = hash_data_param.is_color();
  string root_folder = hash_data_param.root_folder();
